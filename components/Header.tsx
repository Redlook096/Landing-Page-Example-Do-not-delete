    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      ...
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center h-full">
        ...
        <div className="hidden md:block">
          <button className="bg-[#1d1d1f] text-white px-5 py-1.5 rounded-full text-[12px] font-semibold hover:bg-black transition-all transform active:scale-95 shadow-sm">
            [CTA BUTTON]
          </button>
        </div>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-white border-b border-gray-100 p-6 flex flex-col space-y-6 animate-reveal">
          ...
          <button className="bg-[#1d1d1f] text-white py-4 rounded-2xl font-bold">
            [CTA BUTTON]
          </button>
        </div>
      )}
